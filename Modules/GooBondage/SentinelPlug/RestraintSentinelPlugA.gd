...
class_name RestraintSentinelPlugA
...
func doStruggle(_pc, _minigame:MinigameResult):
	...
	if(failChanceLowScore(_pc, 20, _minigame)):
		text = "{user.name} clenches, attempting to push the plug out. Instead, {user.his} efforts cause it to [b]slide deeper[/b], the plug nestling snugly against {user.his} inner walls. It also releases a strong jolt and load of goo deep inside {user.him}."
		damage = -0.5
		lust = scaleDamage(10)
		pain = scaleDamage(10)
		_pc.getBodypart(BodypartSlot.Anus).addFluidOrifice("BlackGoo", RNG.randf_range(80.0, 150.0))
	elif(_handsFree && _armsFree):
		text = "Because {user.name}'s hands are free, {user.he} {user.verbS('ease')} the plug out with little effort."
		damage = 1.0
		lust = scaleDamage(4)
	elif(_legsFree):
		text = "{user.name} squirms and wiggles {user.his} rear, trying to push the plug out of {user.his} butt."
...
func processStruggleTurn(_pc, _isActivelyStruggling):
	if(failChance(_pc, 20)):
		_pc.getBodypart(BodypartSlot.Anus).addFluidOrifice("BlackGoo", RNG.randf_range(30.0, 50.0))
		return {"text": "The Sentinel plug pumps a load of goo into {user.name}'s rear", "lust": scaleDamage(6)}
	elif(failChance(_pc, 40)):
		return {"text": "The Sentinel plug vibrates intensely inside {user.name}'s rear end", "lust": scaleDamage(9)}
	elif(failChance(_pc, 40)):
		return {"text": "The Sentinel plug suddenly zaps {user.name}'s inner walls!", "lust": scaleDamage(3), "pain": scaleDamage(6)}
